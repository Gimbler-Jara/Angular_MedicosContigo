...
export class CitasService {
  ...
  listarCitasAgendadas(idMedico: number): Promise<CitasAgendadasResponseDTO> {
    return lastValueFrom(
      this.http
        .get<CitasAgendadasResponseDTO>(
          `${API}/${CITA_MEDICA}/${ENDPOINTS_CITAS.CITAS_AGENDADAS}/${idMedico}`
        )
        .pipe(
          catchError((error) => {
  ...
  registrarDisponibilidad(
    req: RegistrarDisponibilidadCitaDTO
  ): Promise<EstructuraDisponibilidadCitaResponse> {
  ...
  agendarCita(req: AgendarCitaMedicaDTO): Promise<any> {
    return lastValueFrom(
      this.http
        .post<any>(`${API}/${CITA_MEDICA}/${ENDPOINTS_CITAS.AGENDAR_CITA}`, req)
        .pipe(
          catchError((error) => {
            return throwError(() => error);
          })
  ...
  eliminarCitaReservado(idCita: number): Promise<any> {
    return lastValueFrom(
      this.http
        .delete<any>(`${API}/${CITA_MEDICA}/${ENDPOINTS_CITAS.ELIMINAR_CITA_RESERVADO}/${idCita}`)
        .pipe(
          catchError((error) => {
            return throwError(() => error);
          })
  ...
  listarCitasReservadasPorPaciente(
    idPaciente: number
  ): Promise<CitasReservadasPorPacienteResponseDTO> {
    return lastValueFrom(
      this.http
        .get<CitasReservadasPorPacienteResponseDTO>(`${API}/${CITA_MEDICA}/${ENDPOINTS_CITAS.CITAS_RESERVADAS_POR_PACIENTE}/${idPaciente}`)
        .pipe(
          catchError((error) => {
            return throwError(() => error);
          })
  ...
  marcarcitaComoAtendido( idCita: number, request: DiagnosticoRequestDTO): Promise<any> {
    return lastValueFrom(
      this.http
        .post<any>(`${API}/${CITA_MEDICA}/${ENDPOINTS_CITAS.CAMBIAR_ESTADO_CITA_RESERVADO_ATENDIDO}/${idCita}`, request)
        .pipe(
          catchError((error) => {
            return throwError(() => error);
          })
  ...
  verDetallesDeCitaAtendida(idcita: number): Promise<DetalleCitaAtendidaDTO> {
    ...
    return lastValueFrom(
      this.http.get<DetalleCitaAtendidaDTO>(url).pipe(
        catchError((error) => {
          return throwError(() => error);
        })
      )
